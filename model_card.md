...
**Limitations of this system:** The RAG layer's local knowledge base is small and only covers a handful of genres and moods, so it falls back to a generic note for anything outside that list. I added automated tests for the retrieval layer (`tests/test_retriever.py`), but the generation layer (`rag_explainer.py`) still relies on manual verification and log review, since it depends on a live API call that would need mocking to test automatically — a gap I'd want to close if I extended this project further. This also inherits the same genre and mood-matching biases documented in Section 5 of this model card, since the explanations are grounded in the same underlying scores.

**Could this AI be misused?** The explanation layer could be misused if someone trusted its output as an authoritative music critique rather than a simple, data-grounded explanation of a rule-based score. Since the prompt restricts the model to only the retrieved context, it's less likely to fabricate claims about a song — but it could still be misread as a more sophisticated recommendation than it actually is. A user could also attempt to manipulate the explanations by feeding in fabricated song metadata, since the system doesn't verify the accuracy of the underlying CSV data. To reduce this risk, the system logs every generation call and clearly labels its output as an "AI-Generated Explanation" rather than presenting it as fact, and the fallback path ensures the explanation is always traceable back to the same score breakdown a user can inspect themselves.

**What surprised me while testing reliability:** I expected the API-unavailable fallback to be a rare edge case, but since I built and tested this entire feature without purchasing API credits, the fallback path was actually the *only* path I exercised throughout development. That was a useful surprise — it meant I ended up testing the "unhappy path" far more thoroughly than the "happy path," which gave me more confidence in the system's error handling than I originally expected to have at this stage.